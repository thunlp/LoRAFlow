...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.weight_bias = nn.Parameter(torch.tensor([[-0.5,0.5]]).to(self.lora_fusion_gate.weight.dtype)) 
        self.mlp = LlamaMLP(config)
        self.input_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.seq_num = 1
        self.temperature = 1.0
        self.save_root_path = "/data/public/multilingual/whq/UltraEval/weights_check/single_sample_2"
        self.max_layer_num = config.num_hidden_layers
    ...
    def forward(
        ...
        **kwargs,
        ...
        # if self.layer_idx==0:
        #     import os
        #     os.makedirs(self.save_root_path +  "/seq_"+str(self.seq_num) + "/", exist_ok=True)
        # # import pdb
        # # pdb.set_trace()
        # torch.save(lora_weights,self.save_root_path +  "/seq_"+str(self.seq_num)  +  "/lora_weight_layer_"+str(self.layer_idx)+".pt")
        # ###每一个layer对应的seq_num都是从1开始的，每次调用forward都会增加seq_num
        # self.seq_num+=1
